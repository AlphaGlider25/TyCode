...
pub struct App {
    ...
    // Everything the agent produces is held here until Done, then flushed at once.
    pub pending_messages: Vec<ChatMessage>,
    pub buffered_response: String,
    ...
    pub fn handle_agent_event(&mut self, event: AgentEvent) {
        match event {
            AgentEvent::Thinking => {
                ...
                // Don't clear pending_messages — a multi-turn run reuses the queue.
            }
            AgentEvent::TextDelta(text) => {
                // Accumulate text off-screen — nothing is shown until Done.
                self.buffered_response.push_str(&text);
            }
            AgentEvent::TextDone => {
                // Flush the text buffer into pending (still hidden from chat).
                if !self.buffered_response.is_empty() {
                    self.pending_messages.push(ChatMessage::AssistantText(
                        std::mem::take(&mut self.buffered_response),
                    ));
                }
            }
            AgentEvent::ToolStart { name, input } => {
                // Queue tool call entry (hidden until Done).
                self.pending_messages.push(ChatMessage::ToolCall {
                    name,
            ...
            AgentEvent::ToolResult { name: _, success, output } => {
                // Update the matching pending tool call with its result.
                if let Some(ChatMessage::ToolCall {
                    success: ref mut s,
                    output: ref mut o,
                    ..
                }) = self.pending_messages.iter_mut().rfind(|m| {
            ...
            AgentEvent::Done => {
                ...
                self.messages.append(&mut self.pending_messages);
                self.mode = AppMode::Normal;
                self.set_status("Done — Ready");
                self.scroll_to_bottom();
            }
            AgentEvent::Error(err) => {
                // On error, flush whatever was buffered so the user sees progress.
                self.messages.append(&mut self.pending_messages);
                self.buffered_response.clear();
