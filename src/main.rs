...
async fn run_app(
    ...
    loop {
        ...
        // ── Poll input ───────────────────────────────────────────────────
        let timeout = if matches!(app.mode, AppMode::Processing) {
        ...
        if event::poll(timeout)? {
            match event::read()? {
                ...
                // Mouse events arrive without EnableMouseCapture only on some terminals;
                // keep scroll support for those that forward events anyway.
                Event::Mouse(mouse) => {
            ...
            if state.loading && state.models.is_empty() {
                ...
                tokio::spawn(async move {
                    let models = match provider::create_provider(&config) {
                        Ok(p) => p.available_models().await,
                        Err(_) => vec!["(failed to connect)".into()],
                    };
